package com.yuncai.core.tools.ip;
 ...
 * @version 1.0
 */
public class IpSeeker implements ServletContextAware{
	...
	private static final String IPDATE_FILE = "resources/config/QQWry.Dat";
	...
	private void init() {
		...
		try {
			/*
			 * ServletContextResource cpr = new ServletContextResource
			 * (servletContext,"WEB-INF/temp");
			 * com.cailele.lottery.tools.LogUtil.out("temp
			 * path:"+cpr.getFile()); if(!cpr.getFile().exists()){
			 * cpr.getFile().mkdir(); }
			 * 
			 * InputStream is =
			 * IpSeeker.class.getClassLoader().getResourceAsStream(IPDATE_FILE);
			 * String filePath =
			 * cpr.getFile().getPath()+"\\"+System.currentTimeMillis()+".tmp";
			 * com.cailele.lottery.tools.LogUtil.out("upload
			 * path:"+filePath+".tmp"); FileTools.uploadFile(is, filePath);
			 * ipFile = new RandomAccessFile(filePath, "r"); //
			 */

			ServletContextResource cpr = new ServletContextResource(servletContext, "WEB-INF/classes/" + IPDATE_FILE);
			LogUtil.out("ip db path:" + cpr.getFile());

			ipFile = new RandomAccessFile(cpr.getFile().getPath(), "r");

			// ipFile = new RandomAccessFile(cpr.getFile(), "r");
		} catch (FileNotFoundException e) {
			// log.error("IP地址信息文件没有找到，IP显示功能将无法使用");
			ipFile = null;
			LogUtil.out("IP地址信息文件没有找到，IP显示功能将无法使用");

		} catch (Exception e) {
			ipFile = null;
			LogUtil.out("未知异常[" + e.getMessage() + "]，IP显示功能将无法使用");
		}
		// 如果打开文件成功，读取文件头信息
		if (ipFile != null) {
			try {
	...
	public static void main(String[] args) {
		LogUtil.out(IpSeeker.class.getClassLoader().getResource(IPDATE_FILE).getFile());
		long l = System.currentTimeMillis();
		LogUtil.out(new IpSeeker().getCountry("211.155.21.40"));
		LogUtil.out(System.currentTimeMillis() - l);
		// com.cailele.lottery.tools.LogUtil.out(IPSeeker.getInstance().getArea("127.0.0.1"));
	}
