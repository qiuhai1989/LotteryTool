...
public class SongPiaoFullNetThread extends Thread {
		...
		while (true) {
			if (isRun) {
				for (int lotteryType : lotteryTypeList) {
					...
					if (lotteryType == LotteryType.TCCJDLT.getValue() || lotteryType == LotteryType.QXC.getValue()
							|| lotteryType == LotteryType.SZPL3.getValue() || lotteryType == LotteryType.SZPL5.getValue()) {
						Calendar calendar = Calendar.getInstance();
						int hour = calendar.get(Calendar.HOUR_OF_DAY);
						int minute = calendar.get(Calendar.MINUTE);
						int day = calendar.get(Calendar.DAY_OF_WEEK);
						if ((hour == 19 && minute >= 40) || (hour > 19) || hour < 10) {
							LogUtil.out(type.getName() + " 10:00前 或 19:40后 不送票！");
							continue;
						}
