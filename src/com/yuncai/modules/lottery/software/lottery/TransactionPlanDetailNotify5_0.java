...
public class TransactionPlanDetailNotify5_0 extends TransactionPlanDetailNotify {
...
//						result.setText("<font color='red'>" + "取消" + "</font>");
						result.setText("取消");
					}
//					if(match.getResult()!=null&&match.getResult().trim().equalsIgnoreCase("null:null")){
////						result.setText("<font color='red'>" + "取消" + "</font>");
//						result.setText("取消");
//					}
...
//					lineIds.add(matchItemList.get(i).getLineId()+"");
//					ids.add(bkMatchService.findByPropertys(new String[]{"intTime","lineId"}, new Object[]{matchItemList.get(i).getIntTime(),matchItemList.get(i).getLineId()+""} ).get(0).getId());
				}

				List<BkMatch> matches = new ArrayList<BkMatch>();
				List<BkSpRate>passRates = new ArrayList<BkSpRate>();
//				List<Object[]>matchAndPassRates = bkMatchService.findMatchAndPassRate(ids);
//-----------------------------------------------------------------------				
				//二号方案
...
//-------------------------------------------------------------------------				
				...
				for (int i = 0; i < matches.size(); i++) {
					...
					if(bean.getLotteryType().getValue()==LotteryType.JC_LQ_RFSF.getValue()){
						//让分
						if(passRate.getRangScore()!=0){
							vs.setText(match.getMbName() + "<font color='red'>[" + passRate.getRangScore() + "]</font>" + match.getTgName());
						}
					}else {
						//不让分
					...
					if((match.getMbScore()!=null&&match.getMbScore()>0)&&(match.getTgScore()!=null&&match.getTgScore()>0)){
						result.setText(match.getMbScore()+":"+match.getTgScore());
					}else{
						result.setText("");
					}
//					//比赛取消的情况
