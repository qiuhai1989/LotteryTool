...
public abstract class SportteryBingoCheck<T extends SportteryTicketModel<X>, X extends MatchItem> implements BingoCheck {
	...
	private StringBuffer ticketCountPrize=new StringBuffer();
			...
			for (X x : matchItems) {
				...
				try {// TODO:赛事取消的情况
					...
					if (tempHomeScore.equals("0")) {// 赛事取消
						...
						OptionItem option =null; // CommonUtils.getResultOptionItem(type, matchResultMap);
						...
						if (tempAward.intValue() == 0&&!type.equals("32")) {
							// 结果出来但是sp值未出来
							// 浮动奖与胜分差无关
							isOpenAble = false;
							break;
...
//				}
	...
	public String getTicketCountPrize() {
		return this.ticketCountPrize.toString();
	}
