...
public class FbJq4BingoCheck implements BingoCheck {

	private static final Log log = LogFactory.getLog(FbJq4BingoCheck.class);
		...
		for (int i = 0; i < contentLine.length; i++) {
			...
			if (playType.getValue() == PlayType.DS.getValue()) {
				...
				for (int j = 0; j < lines.length; j++) {
					SFCBingoResult result = BingoUtil.getBingoResultCountBySFC(lines[j], ",", "", resultContent, "\\,", 1, 8);
				...
				SFCBingoResult result = BingoUtil.getBingoResultCountBySFC(content, ",", "", resultContent, "\\,", 1, 8);
	...
	public static void main(String[] args) {
		...
		FbJq4BingoCheck check = new FbJq4BingoCheck();
		HashMap<String, String> openResultMap = new HashMap<String, String>();
		openResultMap.put("prize1", "1");

		openResultMap.put("result", "3,1,0,3,3,1,0,3");

		check.execute("1:3,1,0,3,3,1,0,3~2:3,1,0,3,123,31,01,32", PlayType.HT, 1, openResultMap);
