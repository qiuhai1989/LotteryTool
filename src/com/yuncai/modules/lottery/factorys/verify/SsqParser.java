...
public class SsqParser implements ParserBuilder{
	...
	public ContentCheck checkPlan(String content, double amount) throws Exception {
		...
		for(String str : contentGroup){
			...
			if (playType == PlayType.DS.getValue()) {
				...
				//验证内容
				contentCheck=resultCountext(f,ssqFS(lines[0]),playType);
				...
				//验证内容
				verifyContext=ssqFS(verifyContext);
				contentCheck=resultCountext(f,verifyContext,playType);
				if(!contentCheck.isPass()){
					return contentCheck;
				}
				
				int count = combination(verifyContext.split("\\|")[0].split("\\,").length, 6) * combination(verifyContext.split("\\|")[1].split("\\,").length, 1); ;
				amountTest += count;
                counttest=count;
			}else if(playType==PlayType.DT.getValue()){
				//验证内容
				verifyContext=ssqDT(verifyContext);
				...
				int lendan=blueTuoCount-1;
				int count = combination(tuoCount, 6 - danCount) * combination(blueTuoCount, blueTuoCount-lendan);
...
//			contentCheck.setIsAttribute(add);
	...
	//把内容转换一下
	public static String ssqFS(String content){
	...
	public static String ssqDT(String content){
	...
	public ContentCheck resultCountext(int i,String lines,int playType){
		...
		for(int k = 0; k < pres.length; k++) {
			try {
				...
				if (code < 1 || code > 33) {
					result.setPass(false);
					result.setMessage("第" +  i + "行前区号码必须大于等于1并小于等于33!");
					return result;
				}
		...
		for(int k = 0; k < poss.length; k++) {
			try {
				...
				if(code < 1 || code > 16){
					result.setPass(false);
					result.setMessage("第" +  i + "行后区号码必须大于等于1并小于等于16!");
					return result;
			    }
		...
		if(playType == PlayType.DT.getValue()){
			...
			if(preTuoCount < 2){
				result.setPass(false);
				result.setMessage("第" + i + "行前区拖码个数必须大于等于2!");
				return result;
			}
...
//		System.out.println(dltDT(str));
		...
		SsqParser parser = new SsqParser();
...
//		sb.append("%1-1");
		sb.append("%1");
		ContentCheck check = parser.checkPlan(sb.toString(), 80);
