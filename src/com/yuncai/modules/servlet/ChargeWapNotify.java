...
public class ChargeWapNotify extends HttpServlet {
	...
	protected void execute(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			XFBConfig xfbConfig=(XFBConfig)this.getBean("webConfig");
			...
			String cdkey = request.getParameter("cdkey"); //key值
			String md5key=money+Sid+OrderId+payType+xfbConfig.getChannelId();
			...
			if(md5key.equals(cdkey)){
				if ("11".equals(RespCode)) {
					...
					if (chargeLine.getStatus().getValue() == CommonStatus.YES.getValue()) {// ！！！如果不是处理成功的才处理充值
						...
						try {
							//如果是手机充值96%进行充值
							if(payType_==ChargeType.SZC.getValue()){
								amount_=Math.rint(amount_ * 0.96);
								chargeLine.setAmount(amount_);
								memberChargeLineService.saveOrUpdate(chargeLine);
							}//Float.valueOf(Double.toString(amount_)).intValue()
							memberChargeLineService.charger(WalletType.DUOBAO.getValue(), amount_, "",
									OrderId, request.getRequestURL().toString(), payNo, remoteIp, ct,payType_,0,chargeLine.getChannel());
	...
	public String giftManageLine(Member member,GiftManage bean,Double giftMoney ,MemberWalletService memberWalletService,GiftDatailLineService giftDatailLineService,IpSeeker ipSeeker,MemberOperLineService memberOperLineService,String ip){
			// 保存会员操作流水
